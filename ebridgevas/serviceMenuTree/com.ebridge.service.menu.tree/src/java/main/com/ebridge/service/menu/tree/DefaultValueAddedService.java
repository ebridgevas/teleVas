 ...
 *
 **/
public class DefaultValueAddedService implements ValueAddedService {

    private AtomicReference<ValueAddedService> vasReference = new AtomicReference<>();

    private BundleContext context;
    private ServiceReference serviceReference;

    public DefaultValueAddedService() {
    }

    /**
     * This constructs a proxy shape that lazily gets the value added service.
     *
     * @param valueAddedService The service reference of the service.
     **/
    public DefaultValueAddedService( ValueAddedService valueAddedService ) {
        vasReference.set( valueAddedService );
    }
    ...
    public Response process( Request request, Set<MenuItemDTO> items ) {
        ...
        ValueAddedService valueAddedService = vasReference.get();

        if ( valueAddedService != null ) {

            return valueAddedService.process(request, items);
        }
