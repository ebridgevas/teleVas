...
public class SimulatorPDUProcessor extends PDUProcessor {
	 ...
	 */
	private SmppServerSession session = null;
	 ...
	 */
	private static final String PASSWORD_ATTR = "system-password";

	public SimulatorPDUProcessor( SmppServerSession session,
	...
	public void clientResponse(Response response) {
		...
		System.out.println("SimulatorPDUProcessor.clientResponse() " + response.debugString());
	...
	public void serverRequest(Request request) throws IOException, PDUException {
        ...
        System.out.println("SimulatorPDUProcessor.serverRequest() " + request.debugString());
...
//		Record user = users.find(SYSTEM_ID_ATTR, request.getSystemId());
		Map<String, String> client = clients.get( request.getSystemId() );
		if (client != null) {
			String password = client.get(PASSWORD_ATTR );
			if (password != null) {
